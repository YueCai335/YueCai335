...
**MSc Finance + CFA Level I → Master of Applied Computing @ Wilfrid Laurier University (GPA 11.4 / 12.0)**
...
`Python` `FastAPI` `SQLAlchemy` `Alembic` `PostgreSQL` `pgvector` `SQL` `TypeScript` `Next.js` `React` `pytest` `Vitest` `Docker` `GitHub Actions` `Java`
...
Random Forest trained on PaySim transaction data, served through a Flask REST API, with SHAP explanations surfacing the features behind each prediction.
...
<!--
**YueCai335/YueCai335** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.
